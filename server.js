'use strict';
/* ============================================================================
   Kings Production — Node/Express + SQLite backend
   ----------------------------------------------------------------------------
   Serves the single-file app (index.html) and the /api/* surface backed by
   the shared engine (shared/engine.js) over:
     store — SQLite (server/sqlite-store.js, node:sqlite)
     files — disk uploads (server/disk-files.js)
...
const { createDiskFiles } = require('./server/disk-files.js');
...
/* ---- adapters + engine ---- */
const store = createSqliteStore(path.join(DATA_DIR, 'kings.db'));
const files = createDiskFiles(UPLOAD_DIR);
...
const send = (res, r, okStatus) => res.status(r.ok ? (okStatus || 200) : 400).json(r);
...
/* ---- health ---- */
app.get('/api/health', (req, res) => res.json({ ok: true, data: { name: 'Kings Production API', store: 'sqlite', uptime: Math.round(process.uptime()) } }));
...
const server = app.listen(PORT, () => {
  console.log('┌──────────────────────────────────────────────┐');
  console.log('│  KINGS PRODUCTION  ·  Node/Express + SQLite  │');
  console.log('└──────────────────────────────────────────────┘');
  console.log(`  App:     ${PUBLIC_URL}`);
  console.log(`  API:     ${PUBLIC_URL}/api/health`);
  console.log(`  DB:      ${path.join(DATA_DIR, 'kings.db')}`);
...
function shutdown() {
  console.log('\nShutting down…');
  server.close(() => { try { store.close(); } catch (e) {} process.exit(0); });
